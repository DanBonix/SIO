...
create procedure ajoutSouscrit
	(num int, nom char(20), ville char(20), total decimal(10.2))
begin
	insert into souscrit (num, nom, ville, total) values (num, nom, ville, total);
end !
delimiter ;

-- call ajoutSouscrit(999, 'Brogniart', 'Paris', 100000);
--SELECT* FROM souscrit;
-- show create procedure ajoutSouscrit;

-- 2 --
delimiter !
drop procedure if exists versementBornes !
create procedure versementBornes
	(borneSup int, borneInf int)
begin
	SELECT souscrit.num, nom, ville
	FROM souscrit
	INNER JOIN verse
	ON souscrit.num = verse.num
	WHERE montant < borneSup
	AND souscrit.num IN (SELECT num
						FROM verse
						WHERE montant > borneInf);
end !
delimiter ;

-- call versementBornes(100, 500);

-- 3 --
delimiter !
drop procedure if exists encoreAbonnes !
create procedure encoreAbonnes
	(dateLimite date)
begin
	SELECT distinct num, nom
	FROM souscrit
	WHERE num IN (SELECT num
				FROM abonne
				WHERE date_Add(dateDebut, INTERVAL duree MONTH) > dateLimite);
end !
delimiter ;

-- call encoreAbonnes("2015-02-31");

-- 4 --
delimiter !
drop procedure if exists majVersement !
create procedure majVersement
	(nomSouscripteur char(20), percent decimal(3,1))
begin
	UPDATE souscrit
	SET total = total*(1 + percent/100)
	WHERE nom = nomSouscripteur;
end !
delimiter ;

-- call majVersement("Loisel", -10);

-- 5 --
delimiter !
drop procedure if exists plusGrosVersements !
create procedure plusGrosVersements
	()
begin
	SELECT nom
	FROM souscrit
	WHERE num IN (SELECT num
				FROM verse 
				WHERE montant = (SELECT MAX(totaux.somme)
									FROM(SELECT SUM(montant) AS somme
											FROM verse
											GROUP BY num) totaux)
				);
end !
...
R2 = Max(R1, R1.cumulMontant)
R3 = Selection(R1, cumulMontant = R2.Max)
R4 = Projection(R3, num)

begin


-- call plusGrosVersements();